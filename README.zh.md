...
2. 定时任务 **固定开启**（不再提供开关）
3. 每日开始时间固定为 **01:00**
4. 每日固定执行 **10 次**（浏览 10 个新话题）并自动停止
5. 到点后自动打开 `/latest` 并开始浏览（仅 Linux DO）

**注意**
- 定时任务仅对 **linux.do** 生效
- 默认在站点标签页后台且无操作 10 分钟后启动（近似判断）
...
- 邀请榜单新增“精确 ID（用户名）”输入框。
- 仅在存在有效邀请记录时显示该输入框。
- 输入框样式与现有控制区对齐，避免视觉突兀。
